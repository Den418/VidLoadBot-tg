...
# Семафор: не более 3 одновременных Pyrogram-загрузок,
# чтобы один пользователь с большим файлом не блокировал остальных
...
async def get_all_promos() -> list[tuple]:
    """Вернуть все промокоды: [(code, discount_percent), ...]"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT code, discount_percent FROM promo_codes ORDER BY code") as cur:
...
async def get_promo_discount(code: str) -> int | None:
    """Вернуть процент скидки по коду или None если не найден."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT discount_percent FROM promo_codes WHERE code = ?", (code,)) as cur:
...
def get_periods_keyboard(discount: int = 0) -> InlineKeyboardMarkup:
    ...
    for days, label in PERIOD_LABELS.items():
        ...
        suffix = f" (скидка {discount}%)" if discount else ""
...
def get_payment_methods_keyboard(days: int, discount: int = 0) -> InlineKeyboardMarkup:
    price_stars = int(PRICES_STARS[days] * (1 - discount / 100))
    price_rub   = int(PRICES_RUB[days]   * (1 - discount / 100))
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"⭐ Оплатить Stars ({price_stars} звезд)",   callback_data=f"pay_stars_{days}",
                              icon_custom_emoji_id="5904462880941545555")],
        [InlineKeyboardButton(text=f"💳 Оплатить переводом ({price_rub} ₽)", callback_data=f"pay_manual_{days}",
...
async def period_selected(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        f'<b><tg-emoji emoji-id="5769126056262898415">👛</tg-emoji> Оплата — {label}</b>\n\n'
        f'Стоимость: <b>{rub} ₽</b>  или  <b>{star} ⭐</b>'
    )
    if discount > 0:
        text += (f'\n<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> '
                 f'Скидка по промокоду: <b>{discount}%</b>')
...
async def process_promo(message: Message, state: FSMContext):
    discount = await get_promo_discount(message.text.strip().upper())
    ...
    if discount is not None:
        ...
        await message.answer(
            ...
            f'Промокод активирован! Скидка <b>{discount}%</b></b>\n\n'
            f'Новая цена за {label}: <b>{rub} ₽</b> / <b>{star} ⭐</b>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_payment_methods_keyboard(days, discount))
    else:
        await message.answer(
...
async def pay_manual_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="5769126056262898415">👛</tg-emoji> Оплата переводом — {label}</b>\n\n'
        f'Сумма к оплате: <b>{price} ₽</b>{disc_line}\n'
        f'Реквизиты для перевода:\n<code>{reqs}</code>\n\n'
        f'После оплаты отправьте <b>скриншот чека</b> сюда.',
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="Отмена", callback_data="buy_premium")]])
    )
...
async def receipt_received(message: Message, state: FSMContext):
    ...
    await bot.send_photo(
        ...
        caption=(
            f'<b><tg-emoji emoji-id="5890848474563352982">🪙</tg-emoji> Новая заявка на оплату!</b>\n\n'
            f'👤 ID: <code>{uid}</code>  ({username})\n'
            f'📅 Период: <b>{label}</b>\n'
            f'💰 Сумма: <b>{price} ₽</b>'
        ),
...
async def get_admin_promo_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура управления промокодами: список + кнопки удаления + создать новый."""
    promos = await get_all_promos()
    rows   = []
    for code, pct in promos:
        rows.append([
            InlineKeyboardButton(text=f"🏷 {code} — {pct}%", callback_data=f"promo_info_{code}"),
            InlineKeyboardButton(text="🗑 Удалить",           callback_data=f"promo_del_{code}"),
        ])
...
async def admin_promo(callback: CallbackQuery, state: FSMContext):
    ...
    count  = len(promos)
    text   = (
        f'<b><tg-emoji emoji-id="5886285355279193209">🏷</tg-emoji> Управление промокодами</b>\n\n'
        f'Активных промокодов: <b>{count}</b>\n\n'
        f'{"Нажмите 🗑 рядом с промокодом чтобы удалить его." if count else "Промокодов пока нет."}'
    )
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML,
...
async def promo_delete(callback: CallbackQuery):
    ...
    await callback.answer(f"Промокод {code} удалён", show_alert=False)
    # Обновляем список
    promos = await get_all_promos()
    count  = len(promos)
    text   = (
...
async def promo_create(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != MAIN_ADMIN_ID:
        return
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="5886285355279193209">🏷</tg-emoji> Создание промокода</b>\n\n'
        f'Отправьте код и процент скидки через пробел:\n\n'
        f'Пример: <code>SUMMER 30</code>\n'
        f'<i>Код автоматически переведётся в верхний регистр.</i>',
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def create_promo(message: Message, state: FSMContext):
    ...
    try:
        ...
        lines = "\n".join(
            f"▪ {PERIOD_LABELS[d]}: {int(PRICES_RUB[d]*(1-discount/100))} ₽ / {int(PRICES_STARS[d]*(1-discount/100))} ⭐"
            for d in PERIOD_LABELS
        )
        await message.answer(
            f'<b><tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> '
            f'Промокод <code>{code}</code> со скидкой {discount}% создан!</b>\n\n'
        ...
        await message.answer(
            f'<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> '
            f'Ошибка: {e}\n\nФормат: <code>КОД СКИДКА</code>\nПример: <code>SUMMER 30</code></b>',
            parse_mode=ParseMode.HTML)
    await state.clear()
...
# ✅ ФИКС: опции yt-dlp с платформо-специфичными настройками
def _base_ydl_opts(url: str = "") -> dict:
    ...
    # YouTube — Android-клиент обходит большинство защит без cookies
    ...
    # TikTok — нужен мобильный Safari UA + Referer
    ...
    # Pinterest — yt-dlp иногда требует явного UA
...
def extract_media_info_sync(url: str):
    ...
    """
    opts = _base_ydl_opts(url)  # передаём URL для платформо-специфичных настроек
    ...
    if not unique_res:
        # Форматов нет — пробуем отдать как фото (особенно для Pinterest картинок)
        thumb = info.get('thumbnail') or info.get('url')
        return {"type": "photo", "url": url, "thumb": thumb}
...
async def tiktok_get_info(url: str) -> dict | None:
    """
    Получить информацию о TikTok-видео через TikWM API.
    Возвращает dict с полями: title, thumb, play (прямая ссылка без водяного знака).
    """
    import aiohttp
    ...
    try:
        ...
        if data.get('code') == 0:
            ...
            return {
                ...
                # play — без водяного знака; hdplay — HD без водяного знака (если есть)
...
async def tiktok_download_and_send(url: str, user_id: int, status_msg: Message):
    """Скачать TikTok-видео через TikWM и отправить пользователю."""
    import aiohttp
    ...
    if not info or not info.get('play'):
        await status_msg.edit_text(
            '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Не удалось получить видео с TikTok.</b>\n'
            '<i>Возможно, видео приватное или удалено.</i>',
            parse_mode=ParseMode.HTML)
        return False
    ...
    try:
        await status_msg.edit_text(
            '<b><tg-emoji emoji-id="6039802767931871481">⬇️</tg-emoji> Скачиваю TikTok...</b>',
            parse_mode=ParseMode.HTML)
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[TikTok download] {e}")
        if os.path.exists(file_path):
            os.remove(file_path)
        await status_msg.edit_text(
            '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Ошибка скачивания TikTok.</b>\n'
            '<i>Попробуй ещё раз.</i>',
            parse_mode=ParseMode.HTML)
        return False

    # 3. Водяной знак (если включён)
    if await is_watermark_enabled():
        try:
            await status_msg.edit_text(
                '<b><tg-emoji emoji-id="5870676941614354370">🖋</tg-emoji> Добавляю водяной знак...</b>',
                parse_mode=ParseMode.HTML)
        except Exception:
            pass
    ...
    try:
        await status_msg.edit_text(
            '<b><tg-emoji emoji-id="5963103826075456248">⬆️</tg-emoji> Отправляю...</b>',
            parse_mode=ParseMode.HTML)
    except Exception:
        pass
    ...
    try:
        ...
        if file_size_mb < 50:
            ...
            if pyro_upload_semaphore.locked():
                try:
                    await status_msg.edit_text(
                        '<b><tg-emoji emoji-id="5983150113483134607">⏰</tg-emoji> Очередь на отправку...</b>\n'
                        '<i>Файл отправится автоматически.</i>',
                        parse_mode=ParseMode.HTML)
                except Exception:
                    pass
            async with pyro_upload_semaphore:
                await asyncio.wait_for(
                    pyro_app.send_video(chat_id=user_id, video=file_path),
                    timeout=600)
        ...
        await status_msg.edit_text(
            '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Ошибка при отправке видео.</b>',
            parse_mode=ParseMode.HTML)
        return False
...
async def download_photo_direct(photo_url: str) -> str | None:
    """
    Скачать изображение напрямую по URL (используется для Pinterest и других фото).
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(photo_url, headers=headers, timeout=timeout) as resp:
                if resp.status == 200:
...
async def process_link(message: Message, state: FSMContext):
    ...
    # ── TikTok: используем TikWM API вместо yt-dlp ──────────────────────────
    if is_tiktok_url(message.text):
        ok = await tiktok_download_and_send(message.text, uid, msg)
        if ok and not await is_premium(uid):
    ...
    if media_info["type"] == "photo":
        ...
        # Для Pinterest и других фото-сервисов — скачиваем изображение напрямую по URL превью
        photo_url = media_info.get("thumb")
        if photo_url:
            try:
                file_path = await download_photo_direct(photo_url)
                if file_path and os.path.exists(file_path):
                    await msg.answer_photo(FSInputFile(file_path))
                    os.remove(file_path)
                    await msg.delete()
                ...
                print(f"[photo direct] Ошибка: {e}")

        # Fallback — пробуем через yt-dlp
...
async def download_and_send_media(url: str, user_id: int, status_msg: Message, format_id: str = "best"):
    ...
    try:
        ...
        if file_size_mb < 50:
            ...
            # ✅ ФИКС: большие файлы через Pyrogram + семафор,
            # чтобы один пользователь с большим файлом не блокировал остальных
            ...
            # Если все слоты заняты — уведомляем пользователя
            if pyro_upload_semaphore.locked():
                try:
                    await status_msg.edit_caption(
                        caption=(
                            f'<b><tg-emoji emoji-id="5983150113483134607">⏰</tg-emoji> '
                            f'Очередь на отправку...</b>\n'
                            f'<i>Сервер занят другими загрузками. Ожидайте — файл отправится автоматически.</i>'
                        ),
                        parse_mode=ParseMode.HTML)
                except Exception:
                    pass
...
async def main():
    ...
    # Семафор инициализируется внутри event loop (нельзя создавать глобально в Python 3.10+)
